...
Modified for Kookaberry by Tony Strasser 12 August 2022.
Note the HCSR04 is a 5 volt module and so not compatible with the Kookaberry which uses 3.3 volts.
Instead use the 3.3V RCWL-1601 ultrasonic distance sensor,
optionally with the Quokka QK-02-065 adapter module for convenient connection with the Kookaberry.
'''

class HCSR04:
    """
    Driver to use the untrasonic sensor HC-SR04 (5V) / RCWL-1601 (3.3V-5V).
...
import kooka
sensor = HCSR04(trigger_pin='P3A', echo_pin='P3B')
disp = kooka.display
while True:
    distance, pulse = sensor.distance_mm()
    disp.fill(0)
    disp.text('HSC04 Test', 0, 6)
    disp.text('D: %d mm' % distance, 0, 30 )
    disp.text('P: %d usec' % pulse, 0, 40)
    disp.show()
